...
function ArbCalculator() {
  ...
  const calc = useCallback(() => {
    ...
    // Optimal stake split
    const s1 = stake / (1 + d1 / d2)   // stake on leg 1
    const s2 = stake - s1               // stake on leg 2
    const ret1 = s1 * d1                // return if leg1 wins
    const ret2 = s2 * d2                // return if leg2 wins
  ...
  return (
    <SectionCard icon="📊" title="Bet Performance Calculator" subtitle="Log your bets, track P&L, ROI, win rate, and closing line value over time">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {profitByBet.map(b => (
          <div key={b.id} style={{ display: 'grid', gridTemplateColumns: '2fr 70px 70px 120px 80px 30px', gap: 8, alignItems: 'center', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 9, padding: '10px 14px' }}>
