...
type ThemeCtxType = {
  theme: Theme
  toggle: () => void
  /** Call on pages that must always appear dark (e.g. home page) */
  forceDark: () => void
  /** Call to restore the user's saved preference */
  restoreTheme: () => void
}
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Force dark visually without changing the saved preference
  const forceDark = () => {
    document.documentElement.removeAttribute('data-theme')
  }

  // Restore user's saved preference (call on unmount of forced-dark pages)
  const restoreTheme = () => {
    const s = (localStorage.getItem('to_theme') as Theme) || 'dark'
