...
router.get('/revenue', async (req, res) => {
  try {
    ...
    users.forEach(u => u.payments.forEach(p => { if (byPlan[p.plan] !== undefined) byPlan[p.plan] += p.amount || 0 }))
...
router.get('/payments', async (req, res) => {
  try {
    ...
    // Flatten all payments across all users
    const payments = []
    for (const user of users) {
      for (const p of user.payments || []) {
        if (since && new Date(p.date) < since) continue
        payments.push({
          userName:       user.name,
