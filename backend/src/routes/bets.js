...
router.get('/stats', protect, async (req, res) => {
  try {
    ...
    const bySport = {}
    bets.forEach(b => {
      if (!bySport[b.sport]) bySport[b.sport] = { bets: 0, wins: 0, profit: 0, staked: 0 }
...
router.post('/', protect, async (req, res) => {
  try {
    ...
    if (!game || !market || !book || !odds || !stake) return res.status(400).json({ success: false, message: 'game, market, book, odds, stake required.' })
...
router.put('/:id', protect, async (req, res) => {
  try {
    ...
    allowed.forEach(k => { if (req.body[k] !== undefined) bet[k] = req.body[k] })
    await bet.save()
