...
async function sendOwnerNewSubscriberAlert({ name, email, plan, billingPeriod, price }) {
  const ownerEmail = process.env.OWNER_EMAIL || process.env.ZOHO_USER
  if (!ownerEmail) {
    console.warn('[Email] OWNER_EMAIL not set — skipping new-subscriber owner alert')
  ...
  const html = `
    <div style="font-family:-apple-system,sans-serif;max-width:480px;margin:0 auto;">
      ...
      <p style="color:#64748b;font-size:13px;margin-top:0;">A new paid subscription just started.</p>
