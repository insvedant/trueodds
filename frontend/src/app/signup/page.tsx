...
/* ── Floating stat bubbles for the left panel ── */
const STATS = [
...
const PLAN_DATA = {
  gold: {
    name: 'Gold', price: 15.99, color: '#f0a500', bg: 'rgba(240,165,0,0.08)',
    border: 'rgba(240,165,0,0.35)',
    features: ['Full Arbitrage Finder', '+EV Betting Tools', '40+ US Sportsbooks', 'Smart Alerts', 'Unlimited Tracker'],
  },
  platinum: {
    name: 'Platinum', price: 49.99, color: '#8957e5', bg: 'rgba(137,87,229,0.08)',
    border: 'rgba(137,87,229,0.35)',
    features: ['Everything in Gold', '100+ Global Sportsbooks', 'Live In-Play Odds', 'API Access', '1:1 Coaching'],
  },
}

/* ── Animated card number formatter ── */
      ...
      <div style={{ position: 'relative', width: '100%', height: '100%', transformStyle: 'preserve-3d', transition: 'transform 0.6s cubic-bezier(0.4,0,0.2,1)', transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, backfaceVisibility: 'hidden', background: 'linear-gradient(135deg, #1a1f2e 0%, #2d3748 100%)', borderRadius: 16, padding: 24, boxShadow: '0 20px 60px rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.1)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 28 }}>
            <div style={{ fontWeight: 900, fontSize: 16, color: '#e6edf3', letterSpacing: '-0.5px' }}>True<span style={{ color: '#00C853' }}>Odds</span></div>
...
export default function SignupPage() {
  ...
  // Step 2 — plan
  const [plan, setPlan] = useState<'gold' | 'platinum'>('gold')
  // Step 3 — card
  ...
  const submit = async () => {
    ...
    try {
      ...
      await register(account.name, account.email, account.password, account.phone)

      // Step 2: Create Stripe subscription with trial
      // NOTE: In production, use Stripe.js to tokenise the card client-side
      // and pass the PaymentMethod ID here. For now we send plan selection
      // and the backend creates the subscription in demo mode.
      await api.post('/subscriptions/create-with-trial', {
        planId: plan,
        // paymentMethodId: 'pm_xxx' ← replace with Stripe.js result
        paymentMethodId: `pm_demo_${plan}_${Date.now()}`, // demo placeholder
      })
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: 0 }}>
        ...
        {/* ── LEFT PANEL — betting stats animation ── */}
          ...
          <div style={{ width: '100%', maxWidth: 440 }}>
            ...
            {step === 2 && (
              <div className="step-content">
                ...
                {/* Mandate disclosure — legally required */}
